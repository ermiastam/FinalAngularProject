...
export class JobpostComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private dataService: DataService, private auth:AuthService,private mapsAPILoader: MapsAPILoader,
              private ngZone: NgZone,public route:Router) {
  ...
  onSubmit() {
    ...
    this.route.navigate(['myposts/serachposts'], {queryParams:{email: this.auth.getUser().email}});

   // [routerLink]="['myposts', 'searchposts']" [queryParams]="{email: this.auth.getUser().email}"
